...
def stu_search():
     search_students = []
     print('[ 학생 검색 ] ')
     search = input('찾고자 하는 학생의 이름을 입력해주세요. >> ')
     
     search_cnt = 0 # 학생이 존재하지 않는다면 0, 존재한다면 1
     for s in students:
          if s['name'] == search:
               break
     search_cnt += 1
          
     if search == len(students):# 학생 수가 5명이라면 4번까지, 4번까지 다 돌면 학생이 존재X
          print('찾는 학생이 없습니다. 학생의 이름을 다시 확인하고 입력해주세요.')
          
     else:
          print(f'{search} 학생을 찾았습니다. 성적을 출력합니다.')
          search_students.append(students[search_cnt]) # 1번방에 이름이 있음
          stu_grade_print(search-students) # 1명이 추가 되었고, 그 추가된 학생을 출력해라
